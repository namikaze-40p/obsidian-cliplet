...
export class ClipletStoreJson {
	...
	constructor(private _plugin: Cliplet, private _aesKey: CryptoKey) {
	...
	async list(): Promise<ClipletItem[]> {
		...
		return await Promise.all(cliplets.map(async cliplet => {
			cliplet.content = await crypto.decryptData(cliplet.content, this._aesKey);
			return cliplet;
		}));
	}
	...
	async deleteExceededRecords(maxCount: number): Promise<void> {
		...
		const toDeleteItems = candidates.sort((a, b) => b.latest - a.latest).slice(maxCount).map(({ id }) => id);
		this._plugin.settings.cliplets = this._cliplets.filter(cliplet => !toDeleteItems.includes(cliplet.id));
		await this._plugin.saveSettings();
		this._clipletMap = new Map(this._cliplets.map(cliplet => [cliplet.id, cliplet]));
	}
