...
const compare = (a: ClipletItem, b: ClipletItem): number => {
	...
	return Math.max(b.lastUsed, b.lastModified, b.created) - Math.max(a.lastUsed, a.lastModified,a.created);
}
...
export class ClipletService {
	...
	private constructor(private _appId: string, private _plugin: Cliplet) {}
	...
	async migrationStorageData(): Promise<void> {
		...
		const fromService = this.storageType === 'idb' ? ClipletServiceJson.instance : ClipletServiceIdb.instance;
		const toService = this.storageType === 'idb' ? ClipletServiceIdb.instance : ClipletServiceJson.instance;
